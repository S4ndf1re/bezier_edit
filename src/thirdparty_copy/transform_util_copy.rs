...
pub fn handle_transform_events(
    ...
    mut position_mutator: ResMut<Events<SnapToPosition>>,
    mut rotation_mutator: ResMut<Events<SnapToRotation>>,
) {
        ...
        Ok(mut root_transform) => {
            ...
            match view {
                ...
                None => {
                    ...
                    for event in events {
                        position_mutator.send(event);
                    }

                    let mut events = vec![];
                    for event in rotation_mutator.drain() {
                        events.push(event.clone());
                    }
                    for event in events {
                        rotation_mutator.send(event);
                    }
                }
            }
        }
        Err(_) => error!("error getting root transform"),
    }
}
