...
fn setup(mut commands: Commands, mut rotation_writer: EventWriter<SnapToRotation>, mut position_writer: EventWriter<SnapToPosition>) {
    commands.spawn((
        Camera3d::default(),
        Transform::from_xyz(0.0, 0.0, -10.0).looking_at(Vec3::new(0.0, 0.0, 0.0), Vec3::Y),
    ));
...
#[cfg(feature = "vr_enable")]
fn snap_to_position(mut rotation_writer: EventWriter<SnapToRotation>, mut position_writer: EventWriter<SnapToPosition>) {
    let position = Transform::from_xyz(0.0, 0.0, -10.0).looking_at(Vec3::new(0.0, 0.0, 0.0), Vec3::Y);
...
fn create_app() -> App {
    ...
    app
        ...
        //.add_plugins(AdvancedOrbitControls)
        .add_plugins(TranslationController)
        .add_systems(Startup, (setup, create_actions.before(XRUtilsActionSystemSet::CreateEvents)))
